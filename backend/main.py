...
from grain_analysis import (
    astm_grain_number, segment_grains, segment_from_seeds,
    generate_auto_seeds, calibrate_from_annotation,
)
...
async def analyze_image(file: UploadFile = File(...)) -> JSONResponse:
    ...
    body = response.json()
    print(f"[analyze] Raw VLM body: {json.dumps(body)[:2000]}", flush=True)

    if "error" in body:
        err = body["error"]
        raise HTTPException(status_code=502, detail=f"VLM provider error: {err.get('message', err)}")

    try:
        content = body["choices"][0]["message"]["content"]
    except (KeyError, IndexError, TypeError) as exc:
        raise HTTPException(
            status_code=502, detail=f"VLM unexpected format: {json.dumps(body)[:500]}"
        ) from exc
...
@app.post("/api/grain-seeds")
async def grain_seeds_endpoint(
    file: UploadFile = File(...),
    sensitivity: int = Query(50, ge=1, le=99, description="Seed detection sensitivity (1=coarse, 99=fine)"),
    clahe_clip: float = Query(2.0, ge=1.0, le=5.0, description="CLAHE contrast clip limit"),
) -> JSONResponse:
    if not file.content_type or not file.content_type.startswith("image/"):
        raise HTTPException(status_code=400, detail="Please upload a valid image file.")
    image_bytes = await file.read()
    if not image_bytes:
        raise HTTPException(status_code=400, detail="Uploaded file is empty.")
    image = decode_uploaded_image(image_bytes)
    seeds = generate_auto_seeds(image, sensitivity=sensitivity, clahe_clip=clahe_clip)
    return JSONResponse(content={"seeds": seeds, "count": len(seeds)})


from fastapi import Form as FastAPIForm

@app.post("/api/grain-contours")
async def grain_contours(
    file: UploadFile = File(...),
    seed_sensitivity: int = Query(50, ge=1, le=99, description="Auto-seed sensitivity (1=coarse, 99=fine)"),
    min_grain_px: int = Query(100, ge=10, le=2000, description="Minimum grain area in pixels"),
    clahe_clip: float = Query(2.0, ge=1.0, le=5.0, description="CLAHE contrast clip limit"),
    seeds_json: str = FastAPIForm("[]", description="JSON array of {x,y} seed coords (normalised 0–1)"),
) -> JSONResponse:
    if not file.content_type or not file.content_type.startswith("image/"):
        raise HTTPException(status_code=400, detail="Please upload a valid image file.")
    image_bytes = await file.read()
    ...
    try:
        seeds: list = json.loads(seeds_json)
    ...
    result = segment_from_seeds(image, seeds, clahe_clip=clahe_clip, min_grain_px=min_grain_px)
    return JSONResponse(content=result)


@app.post("/api/grain-template")
async def grain_template(
    file: UploadFile = File(...),
    x0: float = Query(..., ge=0.0, le=1.0, description="Normalised left edge of annotation"),
    y0: float = Query(..., ge=0.0, le=1.0, description="Normalised top edge of annotation"),
    x1: float = Query(..., ge=0.0, le=1.0, description="Normalised right edge of annotation"),
    y1: float = Query(..., ge=0.0, le=1.0, description="Normalised bottom edge of annotation"),
) -> JSONResponse:
    """
    Accept an image and a normalised rectangle [x0,y0]→[x1,y1] marking ONE grain.
    Trains a one-shot RF model on real pixels from that grain and returns
    calibrated segmentation parameters.
    """
    if not file.content_type or not file.content_type.startswith("image/"):
        raise HTTPException(status_code=400, detail="Please upload a valid image file.")
    image_bytes = await file.read()
    ...
    image = decode_uploaded_image(image_bytes)
    try:
        params = calibrate_from_annotation(image, x0, y0, x1, y1)
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    # Run segmentation with calibrated params and return seeds too
    seeds = generate_auto_seeds(image, sensitivity=params["seed_sensitivity"], clahe_clip=params["clahe_clip"])
    result = segment_from_seeds(image, seeds, clahe_clip=params["clahe_clip"], min_grain_px=params["min_grain_px"])
    return JSONResponse(content={**params, **result})
